...
function Client(key, secret, host, timeout) {
    ...
    if (!key || !secret) {
        if (process.env.CREATOR_LOGIN_KEY && process.env.CREATOR_LOGIN_SECRET) {
            this.authKey = process.env.CREATOR_LOGIN_KEY;
            this.authSecret = process.env.CREATOR_LOGIN_SECRET;
            // this.authorization = Authenticator.authenticate(key, secret); // todo: promise
        }
